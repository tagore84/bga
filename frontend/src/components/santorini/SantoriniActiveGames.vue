<template>
  <div class="active-games-container">
    <div class="header-actions">
       ...
       <h2 class="page-title">Partidas de Santorini</h2>
    ...
    <div v-else class="games-grid">
      ...
      <div v-else v-for="game in games" :key="game.id" class="glass-panel game-card">
        <div class="card-header">
          ...
          <span class="turn-badge" :class="game.current_turn.toLowerCase()">Turno: {{ game.current_turn === 'p1' ? 'AZUL' : 'ROJO' }}</span>
        </div>
        
        <div class="card-body">
             <div class="info-row">
               <span>Azul (P1):</span> <strong class="blue-player">🔵 {{ game.player_p1_name || 'Desconocido' }}</strong>
             </div>
             <div class="info-row">
               <span>Rojo (P2):</span> <strong class="red-player">🔴 {{ game.player_p2_name || 'Desconocido' }}</strong>
             </div>
             <div class="info-row">
               <span>Estado:</span> <strong>{{ formatStatus(game.status) }}</strong>
             </div>
        </div>
...
async function deleteGame(id) {
  ...
  try {
    const res = await fetch(`${API_BASE}/santorini/${id}`, {
      method: 'DELETE',
      headers: {
...
function formatStatus(status) {
  ...
  if (status === 'p1_won') return 'Victoria Azul'
  if (status === 'p2_won') return 'Victoria Roja'
  return status
}
...
/* Adaptation for Santorini P1/P2 */
.turn-badge.p2 { background: #ff6b6b; color: white; }
.turn-badge.p1 { background: #4dabf7; color: white; }
