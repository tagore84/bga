<template>
  <div class="active-games-container">
    <div class="header-actions">
       ...
       <h2 class="page-title">Partidas de Wythoff</h2>
    ...
    <div v-else class="games-grid">
      ...
      <div v-else v-for="game in games" :key="game.id" class="glass-panel game-card">
        <div class="card-header">
          ...
          <span class="turn-badge" :class="game.current_turn === '1' ? 'p1' : 'p2'">
            Turno: {{ game.current_turn === '1' ? 'JUGADOR 1' : 'JUGADOR 2' }}
          </span>
        </div>
        
        <div class="card-body">
             <div class="info-row">
               <span>Jugador 1 (Inicia):</span> <strong class="p1-player">🟡 {{ game.player_1_name || 'Desconocido' }}</strong>
             </div>
             <div class="info-row">
               <span>Jugador 2:</span> <strong class="p2-player">🟣 {{ game.player_2_name || 'Desconocido' }}</strong>
             </div>
             <div class="info-row">
...
async function deleteGame(id) {
  ...
  try {
    const res = await fetch(`${API_BASE}/wythoff/${id}`, {
      method: 'DELETE',
      headers: {
...
const STATUS_MAP = {
  ...
  '1_won': 'Ganó Jugador 1',
  '2_won': 'Ganó Jugador 2',
  'finished': 'Finalizada',
  'waiting': 'Esperando'
}
...
.turn-badge.p1 { background: #facc15; color: black; } /* Yellow-400 equivalent */
.turn-badge.p2 { background: #a855f7; color: white; } /* Purple-500 equivalent */
...
.p1-player { color: #facc15; }
.p2-player { color: #a855f7; }
